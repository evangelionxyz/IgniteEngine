...
static float3 LightFalloff(float3 lightIntensity, float3 fallOff, float3 lightPosition, float3 position)
{
...
static float TRDistribution(float3 normal, float3 halfVector, float roughness)
{
...
static float GGXVisiblity(float3 normal, float3 lightDirection, float3 viewDirection, float roughness)
{
...
static float3 GGXReflect(float3 normal, float3 reflectDirection, float3 viewDirection, in float3 reflectRadiance, float3 specularColor, float roughness)
{
...
static float3 BinnPhong(float3 normal, float3 lightDirection, float3 viewDirection, float3 lightIrradiance, float3 diffuseColor, float3 specularColor, float roughness)
{
...
static float3 GGX(float3 normal, float3 lightDirection, float3 viewDirection, float3 lightIrradiance, float3 diffuseColor, float3 specularColor, float roughness)
{
    float3 diffuse = diffuseColor * M_RCPPI;
    float3 halfVector = normalize(viewDirection + lightDirection);
...
static float3 TextureEmissive(float3 diffuseColor, float emissive)
{
