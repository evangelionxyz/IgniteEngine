...
namespace ignite
{
    ...
    void WidgetRenderer::RenderWidgetItems()
    {
        ...
        for (const WidgetRenderLayer &layer : m_RenderLayers)
        {
            ...
            // Collect root-level items (items without a parent) and sort by zIndex
            std::vector<Ref<IWidgetItem>> rootItems;
            rootItems.reserve(layer.widget->GetItems().size());
            ...
            // Stable sort so items with the same zIndex keep their insertion order
            std::stable_sort(rootItems.begin(), rootItems.end(),
                [](const Ref<IWidgetItem> &a, const Ref<IWidgetItem> &b)
                {
                    return a->zIndex < b->zIndex;
                });

            for (const Ref<IWidgetItem> &item : rootItems)
                item->Draw(this, assetManager);
        }
    }
